...
public class EventoFinControlReloj : Evento
{
    ...
    public override void GenerarEvento(ref VectorEstado vE)
    {
        ...
        TiempoNeto1 = Math.Truncate(Math.Sqrt(-2*Math.Log(1-Rnd1))*Math.Cos(2* Math.PI*Rnd2) * 100) / 100;
        TiempoNeto2 = Math.Truncate(Math.Sqrt(-2*Math.Log(1-Rnd1))*Math.Sin(2* Math.PI*Rnd2) * 100) / 100;
